...
export default function CreateToilet({close}) {

    const {data, setData, post, reset, errors} = useForm({ name: '', length: '', width: '', floor_location: '', description: '',  });

    const onChange = (e) => setData({ ...data, [e.target.id]: e.target.value });

    const onSubmit = (e) => {
        e.preventDefault();
        post(route('toilets.store'), {
            data, 
            onSuccess: () => {
                reset(),
                close()
            ...
            <form onSubmit={onSubmit}>
                <div className="modal-body">
                        ...
                        <div className="form-group">
                            ...
                            {errors && <div className='text-danger mt-1'>{errors.floor_location}</div>}
                        </div>
