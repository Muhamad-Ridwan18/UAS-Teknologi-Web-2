...
export default function EditClassroom({close, model}) {
    ...
    const {data, setData, put, reset, errors} = useForm({ name: model.name, capacity: model.capacity, floor: model.floor, });
    ...
    useEffect(() => {
        setData({...data,
            name: model.name, capacity: model.capacity, floor: model.floor
        });
    }, [model]);
